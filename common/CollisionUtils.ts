...
class CollisionUitls {
    ...
    public static getRectangleVerPoints_txty(shap: egret.DisplayObject, tx_, ty_): IPoint[] {
     ...
     * 获取多边形顶点旋转后的位置
     */
    public static getPolygonVerPoints(shap: egret.DisplayObject, point: IPoint[], _parent?: egret.DisplayObject) {
        const anX = shap.anchorOffsetX;
        const anY = shap.anchorOffsetY;
        ...
        let ty ;
        if (_parent) {
            let p_gl = shap.parent.localToGlobal(mat2x3.tx, mat2x3.ty);
            let p_ll = _parent.stage.globalToLocal(p_gl.x, p_gl.y);
            tx = p_ll.x;
            ty = p_ll.y;
        } else {
            tx = mat2x3.tx;
        ...
        for (let i of point) {
            let px = i.x - anX;
            let py = i.y - anY;
            const p1 = { x: tx + a * px + c * py, y: ty + b * px + d * py };
            arr.push(p1)
        }
        return arr;
    }
